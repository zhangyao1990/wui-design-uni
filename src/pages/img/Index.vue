 ...
 * @Description:
 * @FilePath: \wui-design-uni\src\pages\img\Index.vue
 * 记得注释
-->
<template>
  <page-wraper>
    ...
    <demo-block title="填充">
      <view class="col" v-for="(mode, index) in modes" :key="index">
        <wui-img width="100%" height="27vw" :src="joy" :mode="mode" />
        <view class="center">{{ mode }}</view>
      </view>
    </demo-block>
    <demo-block title="圆形">
      <view class="col" v-for="(mode, index) in modes" :key="index">
        <wui-img round width="100%" height="27vw" :src="joy" :mode="mode" />
        <view class="center">{{ mode }}</view>
      </view>
    </demo-block>
    <demo-block title="圆角">
      <view class="col" v-for="(mode, index) in modes" :key="index">
        <wui-img width="100%" height="27vw" :radius="5" :src="joy" :mode="mode" />
        <view class="center">{{ mode }}</view>
      </view>
    </demo-block>
...
import type { ImageMode } from '@/uni_modules/wui-design-uni/components/wui-img/types'
