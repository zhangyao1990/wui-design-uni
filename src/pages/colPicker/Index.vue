  ...
  <page-wraper>
    ...
    <view style="margin: 20px 0">
      <wui-cell-group border>
        <wui-col-picker label="选择地址" v-model="value1" :columns="areaData1" :column-change="columnChange1" @confirm="handleValue" />
        <wui-col-picker label="初始选项" v-model="value2" :columns="areaData2" :column-change="columnChange" auto-complete />
        <wui-col-picker label="禁用" disabled v-model="value3" :columns="areaData3" :column-change="columnChange1" />
        <wui-col-picker label="只读" readonly v-model="value3" :columns="areaData3" :column-change="columnChange1" />
        <wui-col-picker label="禁用选项" v-model="value4" :columns="areaData4" :column-change="columnChange1" />
        <wui-col-picker label="选项提示信息" v-model="value5" :columns="areaData5" :column-change="columnChange1" />
        <wui-col-picker label="展示格式化" v-model="value6" :columns="areaData3" :column-change="columnChange1" :display-format="displayFormat" />
        <wui-col-picker label="标题" v-model="value7" title="选择地址" :columns="areaData1" :column-change="columnChange1" />
        <wui-col-picker label="before-confirm" v-model="value8" :columns="areaData1" :column-change="columnChange1" :before-confirm="beforeConfirm" />
        <wui-col-picker label="错误" error v-model="value9" :columns="areaData1" :column-change="columnChange1" />
        <wui-col-picker label="必填" required v-model="value10" :columns="areaData1" :column-change="columnChange1" />
      </wui-cell-group>
    </view>
    <demo-block title="一般column-change是个异步获取数据的操作，触发column-change组件会有默认loading，数据响应后关闭loading" transparent>
      <wui-col-picker label="选择地址" v-model="value11" :columns="areaData1" :column-change="columnChange2" />
    </demo-block>
    <demo-block title="label不传" transparent>
      <wui-col-picker v-model="value12" :columns="areaData1" :column-change="columnChange1" />
    </demo-block>
    <demo-block title="大小" transparent>
      <wui-col-picker label="选择地址" v-model="value13" size="large" :columns="areaData1" :column-change="columnChange1" />
    </demo-block>
    <demo-block title="值靠右展示" transparent>
      <wui-col-picker label="选择地址" align-right v-model="value14" :columns="areaData1" :column-change="columnChange1" />
    </demo-block>
...
import type { ColPickerColumnChange } from '@/uni_modules/wui-design-uni/components/wui-col-picker/types'
