    ...
    <page-wraper>
      <view class="icon">
        <view style="position: sticky; top: 0; z-index: 2">
          <wui-search hide-cancel placeholder="查找图标" light v-model="keyword" @search="handleSearch" @clear="handleClear" />
        </view>
        <view class="icon-list">
          <view v-for="(icon, index) in showIcons" :key="index" class="icon-item" @click="handleClick(icon)">
            <view><wui-icon :name="icon" size="22px" custom-class="icon-item-class" /></view>
            <view class="icon-item-name">{{ icon }}</view>
          </view>
          <wui-status-tip v-if="!showIcons.length" image="search" tip="当前无相关图标" />
        </view>
      </view>
    </page-wraper>
  </view>
...
import { useNotify } from '@/uni_modules/wui-design-uni'
  ...
  uni.setClipboardData({
    ...
    success: () => {
      showNotify({
        type: 'success',
        duration: 1500,
        message: `复制成功: <wui-icon name="${icon}" size="22px"></wui-icon>`
      })
    }
  })
