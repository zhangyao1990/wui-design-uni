<template>
  <page-wraper>
    <demo-block :title="`基本用法，数值: ${value1}`">
      <wui-picker-view v-model="value1" :columns="columns1" />
    </demo-block>

    <demo-block :title="`禁用选项，数值: ${value2}`">
      <wui-picker-view v-model="value2" :columns="columns2" />
    </demo-block>

    <demo-block :title="`立即触发 change，数值: ${value6}`">
      <wui-picker-view v-model="value6" :immediate-change="true" :columns="columns2" />
    </demo-block>

    <demo-block :title="`加载中，数值: ${value3}`">
      <wui-picker-view v-model="value3" :columns="columns3" loading />
    </demo-block>

    <demo-block :title="`多列，数值: [${value4}]`">
      <wui-picker-view v-model="value4" :columns="columns4" />
    </demo-block>

    <demo-block :title="`多级联动，数值: [${value5}]`">
      <wui-picker-view v-model="value5" :columns="columns5" :column-change="onChangeDistrict" />
    </demo-block>
  </page-wraper>
</template>
<script lang="ts" setup>
import type { PickerViewColumnChange } from '@/uni_modules/wui-design-uni/components/wui-picker-view/types'
import { ref } from 'vue'
