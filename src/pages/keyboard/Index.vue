  ...
  <page-wraper>
    <demo-block title="基本用法" transparent>
      <wui-cell-group border>
        ...
        <wui-cell title="车牌号键盘" is-link @click="showKeyBoard(10)" />
        <wui-cell title="双向绑定" clickable :value="value1" @click="showKeyBoard(7)" />
        <wui-cell title="展示蒙层" clickable @click="showKeyBoard(8)" />
      </wui-cell-group>
    </demo-block>

    <wui-keyboard v-model:visible="visible1" @input="onInput" @delete="onDelete"></wui-keyboard>
    <wui-keyboard v-model:visible="visible2" mode="custom" extra-key="." close-text="完成" @input="onInput" @delete="onDelete"></wui-keyboard>
    <wui-keyboard v-model:visible="visible3" extra-key="X" close-text="完成" @input="onInput" @delete="onDelete" />
    <wui-keyboard v-model:visible="visible4" title="输入密码" extra-key="." close-text="完成" @input="onInput" @delete="onDelete"></wui-keyboard>
    <wui-keyboard v-model:visible="visible9" extra-key="." close-text="完成" @input="onInput" @delete="onDelete">
      <template #title>
        <text style="color: red">自定义标题</text>
      </template>
    </wui-keyboard>

    <wui-keyboard v-model:visible="visible5" mode="custom" :extra-key="['00', '.']" close-text="完成" @input="onInput" @delete="onDelete" />

    <wui-keyboard v-model:visible="visible6" random-key-order @input="onInput" @delete="onDelete" />
    ...
    <wui-keyboard :modal="true" v-model:visible="visible8" @input="onInput" @delete="onDelete" />

    <wui-keyboard v-model:visible="visible10" mode="car" @input="onInput" @delete="onDelete" />
  </page-wraper>
</template>
