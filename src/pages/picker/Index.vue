  ...
  <page-wraper>
    ...
    <demo-block transparent>
      <wui-cell-group border>
        <wui-picker label="单列选项" v-model="value0" :columns="columns0" />
        <wui-picker label="禁用" disabled v-model="value1" :columns="columns1" />
        <wui-picker label="只读" readonly v-model="value2" :columns="columns2" />
        <wui-picker label="loading" v-model="value3" loading :columns="columns3" />
        <wui-picker label="多列" v-model="value4" :columns="columns4" />
        <wui-picker label="多级联动" v-model="value5" :columns="columns5" :column-change="onChangeDistrict" />
        <wui-picker label="分隔符" v-model="value6" :columns="columns6" :display-format="displayFormat" />
        <wui-picker label="标题" v-model="value9" :columns="columns7" title="文案标题" />
        <wui-picker label="before-confirm" :columns="columns0" v-model="value7" :before-confirm="beforeConfirm" />
        <wui-picker label="错误" v-model="value10" error :columns="columns0" />
        <wui-picker label="必填" v-model="value11" :columns="columns0" required />
        <wui-picker label="可清空" :clearable="true" v-model="value15" :columns="columns5" :column-change="onChangeDistrict" />
      </wui-cell-group>
    </demo-block>
    <demo-block title="label 不传" transparent>
      <wui-picker :columns="columns0" v-model="value12" />
    </demo-block>
    <demo-block title="大小" transparent>
      <wui-picker label="单列选项" v-model="value13" size="large" :columns="columns0" />
    </demo-block>
    <demo-block title="值靠右显示" transparent>
      <wui-picker label="单列选项" v-model="value14" align-right :columns="columns0" />
    </demo-block>
    <demo-block title="默认插槽" transparent>
      <view class="default-slot">
        ...
        <wui-picker :columns="columns0" v-model="value8" use-default-slot @confirm="handleConfirm">
...
import type { ColumnItem, PickerViewColumnChange } from '@/uni_modules/wui-design-uni/components/wui-picker-view/types'
import type { PickerBeforeConfirm, PickerDisplayFormat } from '@/uni_modules/wui-design-uni/components/wui-picker/types'
import { ref } from 'vue'
