    ...
    >
      <view :class="rootClass">
        <view :class="bodyClass">
          <view v-if="title" class="wui-message-box__title">
            {{ title }}
          </view>
          <view class="wui-message-box__content">
            <block v-if="type === 'prompt'">
              <wui-input v-model="inputValue" :type="inputType" size="large" :placeholder="inputPlaceholder || '请输入'" @input="inputValChange" />
              <view v-if="showErr" class="wui-message-box__input-error">
        ...
        <view :class="`wui-message-box__actions ${showCancelButton ? 'wui-message-box__flex' : 'wui-message-box__block'}`">
          <wui-button type="info" block v-if="showCancelButton" custom-style="margin-right: 16px;" @click="toggleModal('cancel')">
            {{ cancelButtonText || translate('cancel') }}
          </wui-button>
          <wui-button block @click="toggleModal('confirm')">
...
const rootClass = computed(() => {
  return `wui-message-box__container ${props.customClass}`
})

const bodyClass = computed(() => {
  return `wui-message-box__body ${!title.value ? 'is-no-title' : ''} ${type.value === 'prompt' ? 'is-prompt' : ''}`
})
