<template>
  <view :class="`wui-select-picker ${cell.border.value ? 'is-border' : ''} ${customClass}`" :style="customStyle">
    <view class="wui-select-picker__field" @click="open">
      <slot v-if="useDefaultSlot"></slot>
      <view
        v-else
        :class="`wui-select-picker__cell ${disabled && 'is-disabled'} ${readonly && 'is-readonly'} ${alignRight && 'is-align-right'} ${
          error && 'is-error'
        } ${size && 'is-' + size}`"
      >
        <view
          v-if="label || useLabelSlot"
          :class="`wui-select-picker__label ${isRequired && 'is-required'} ${customLabelClass}`"
          :style="labelWidth ? 'min-width:' + labelWidth + ';max-width:' + labelWidth + ';' : ''"
        >
          <block v-if="label">{{ label }}</block>
          <slot v-else name="label"></slot>
        </view>
        <view class="wui-select-picker__body">
          <view class="wui-select-picker__value-wraper">
            <view
              :class="`wui-select-picker__value ${ellipsis && 'is-ellipsis'} ${customValueClass} ${
                showValue ? '' : 'wui-select-picker__value--placeholder'
              }`"
            >
              {{ showValue || placeholder || translate('placeholder') }}
            </view>
            <wui-icon v-if="showArrow" custom-class="wui-select-picker__arrow" name="arrow-right" />
            <view v-else-if="showClear" @click.stop="handleClear">
              <wui-icon custom-class="wui-select-picker__clear" name="error-fill" />
            </view>
          </view>

          <view v-if="errorMessage" class="wui-select-picker__error-message">{{ errorMessage }}</view>
        </view>
      </view>
    </view>
    <wui-action-sheet
      ...
      custom-header-class="wui-select-picker__header"
    >
      <wui-search
        v-if="filterable"
        v-model="filterVal"
      ...
      />
      <scroll-view
        :class="`wui-select-picker__wrapper ${filterable ? 'is-filterable' : ''} ${loading ? 'is-loading' : ''} ${customContentClass}`"
        :scroll-y="!loading"
        :scroll-top="scrollTop"
        :scroll-with-animation="true"
      >
        <!-- 多选 -->
        <view v-if="type === 'checkbox' && isArray(selectList)" id="wui-checkbox-group">
          <wui-checkbox-group v-model="selectList" cell :size="selectSize" :checked-color="checkedColor" :min="min" :max="max" @change="handleChange">
            <view v-for="item in filterColumns" :key="item[valueKey]" :id="'check' + item[valueKey]">
              <wui-checkbox :modelValue="item[valueKey]" :disabled="item.disabled">
                <block v-if="filterable && filterVal">
                  <block v-for="text in item[labelKey]" :key="text.label">
                    <text v-if="text.type === 'active'" class="wui-select-picker__text-active">{{ text.label }}</text>
        ...
        <!-- 单选 -->
        <view v-if="type === 'radio' && !isArray(selectList)" id="wui-radio-group">
          <wui-radio-group v-model="selectList" cell :size="selectSize" :checked-color="checkedColor" @change="handleChange">
            <view v-for="(item, index) in filterColumns" :key="index" :id="'radio' + item[valueKey]">
              <wui-radio :value="item[valueKey]" :disabled="item.disabled">
                <block v-if="filterable && filterVal">
                  <block v-for="text in item[labelKey]" :key="text.label">
                    <text :clsss="`${text.type === 'active' ? 'wui-select-picker__text-active' : ''}`">{{ text.label }}</text>
                  </block>
                </block>
                <block v-else>
                  {{ item[labelKey] }}
        ...
        <view v-if="loading" class="wui-select-picker__loading" @touchmove="noop">
          <wui-loading :color="loadingColor" />
        </view>
      </scroll-view>
      <!-- 确认按钮 -->
      <view v-if="showConfirm" class="wui-select-picker__footer">
        <wui-button block size="large" @click="onConfirm" :disabled="loading">{{ confirmButtonText || translate('confirm') }}</wui-button>
      </view>
    </wui-action-sheet>
  </view>
</template>
...
import wuiCheckbox from '../wui-checkbox/wui-checkbox.vue'
...
import wuiButton from '@/uni_modules/wui-design-uni/components/wui-button/wui-button.vue'
...
function setScrollIntoView() {
  ...
  if (isDef(selectList.value) && selectList.value !== '' && !isArray(selectList.value)) {
    wraperSelector = '#wui-radio-group'
    ...
    wraperSelector = '#wui-checkbox-group'
  }
  if (wraperSelector) {
