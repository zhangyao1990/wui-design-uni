<template>
  <!-- 绘制的图片canvas -->
  <view v-if="modelValue" :class="`wui-img-cropper ${customClass}`" :style="customStyle" @touchmove="preventTouchMove">
    <!-- 展示在用户面前的裁剪框 -->
    <view class="wui-img-cropper__wrapper">
      <!-- 画出裁剪框 -->
      <view class="wui-img-cropper__cut">
        <!-- 上方阴影块 -->
        <view :class="`wui-img-cropper__cut--top ${IS_TOUCH_END ? '' : 'is-hightlight'}`" :style="`height: ${cutTop}px;`"></view>
        <view class="wui-img-cropper__cut--middle">
          <!-- 左侧阴影块 -->
          <view
            :class="`wui-img-cropper__cut--left ${IS_TOUCH_END ? '' : 'is-hightlight'}`"
            :style="`width: ${cutLeft}px; height: ${cutWidth}px;`"
          ></view>
          <!-- 裁剪框 -->
          <view class="wui-img-cropper__cut--body" :style="`width: ${cutWidth}px; height: ${cutHeight}px;`">
            <!-- 内部网格线 -->
          ...
          <!-- 右侧阴影块 -->
          <view :class="`wui-img-cropper__cut--right ${IS_TOUCH_END ? '' : 'is-hightlight'}`"></view>
        </view>

        <!-- 底部阴影块 -->
        <view :class="`wui-img-cropper__cut--bottom ${IS_TOUCH_END ? '' : 'is-hightlight'}`"></view>
      </view>
      <!-- 展示的传过来的图片: 控制图片的旋转角度(rotate)、缩放程度(imgScale)、移动位置(translate) -->
      <image
        :prop="isAnimation"
        :change:prop="animation ? animation.setAnimation : ''"
        class="wui-img-cropper__img"
        :src="imgSrc"
        :style="imageStyle"
        :lazy-load="false"
    ...
    />
    <!-- 下方按钮 -->
    <view class="wui-img-cropper__footer">
      <wui-icon custom-class="wui-img-cropper__rotate" v-if="!disabledRotate" name="rotate" @click="handleRotate"></wui-icon>
      <view class="wui-img-cropper__footer--button">
        <view class="is-cancel" @click="handleCancel">{{ cancelButtonText || translate('cancel') }}</view>
        <wui-button size="small" :custom-style="buttonStyle" @click="handleConfirm">{{ confirmButtonText || translate('confirm') }}</wui-button>
      </view>
    </view>
  </view>
</template>
