<template>
  <view :class="`wui-calendar ${cell.border.value ? 'is-border' : ''} ${customClass}`">
    <view class="wui-calendar__field" @click="open">
      ...
      >
        <view
          ...
          :class="`wui-calendar__label ${isRequired ? 'is-required' : ''} ${customLabelClass}`"
          :style="labelWidth ? 'min-width:' + labelWidth + ';max-width:' + labelWidth + ';' : ''"
        >
          <block v-if="label">{{ label }}</block>
          <slot v-else name="label"></slot>
        </view>
        <view class="wui-calendar__body">
          <view class="wui-calendar__value-wraper">
            <view
              :class="`wui-calendar__value ${ellipsis ? 'is-ellipsis' : ''} ${customValueClass} ${
                showValue ? '' : 'wui-calendar__value--placeholder'
              }`"
            >
              {{ showValue || placeholder || translate('placeholder') }}
            </view>
            <wui-icon v-if="!disabled && !readonly" custom-class="wui-calendar__arrow" name="arrow-right" />
          </view>
          <view v-if="errorMessage" class="wui-calendar__error-message">{{ errorMessage }}</view>
        </view>
      </view>
    </view>
    <wui-action-sheet
    ...
    >
      <view class="wui-calendar__header">
        <view v-if="!showTypeSwitch && shortcuts.length === 0" class="wui-calendar__title">{{ title || translate('title') }}</view>
        <view v-if="showTypeSwitch" class="wui-calendar__tabs">
          <wui-tabs ref="calendarTabs" v-model="currentTab" @change="handleTypeChange">
            <wui-tab :title="translate('day')" :name="translate('day')" />
            <wui-tab :title="translate('week')" :name="translate('week')" />
            <wui-tab :title="translate('month')" :name="translate('month')" />
          </wui-tabs>
        </view>
        <view v-if="shortcuts.length > 0" class="wui-calendar__shortcuts">
          <wui-tag
            v-for="(item, index) in shortcuts"
            :key="index"
            custom-class="wui-calendar__tag"
            type="primary"
            plain
            round
        ...
        <wui-icon custom-class="wui-calendar__close" name="add" @click="close" />
      </view>
      <view
        v-if="inited"
        :class="`wui-calendar__view  ${currentType.indexOf('range') > -1 ? 'is-range' : ''} ${showConfirm ? 'is-show-confirm' : ''}`"
      >
        <view v-if="range(type)" :class="`wui-calendar__range-label ${type === 'monthrange' ? 'is-monthrange' : ''}`">
          <view
            :class="`wui-calendar__range-label-item ${!calendarValue || !isArray(calendarValue) || !calendarValue[0] ? 'is-placeholder' : ''}`"
            style="text-align: right"
          >
            {{ rangeLabel[0] }}
          </view>
          <view class="wui-calendar__range-sperator">/</view>
          <view :class="`wui-calendar__range-label-item ${!calendarValue || !isArray(calendarValue) || !calendarValue[1] ? 'is-placeholder' : ''}`">
            {{ rangeLabel[1] }}
          </view>
        </view>
        <wui-calendar-view
      ...
      <view v-if="showConfirm" class="wui-calendar__confirm">
        <wui-button block :disabled="confirmBtnDisabled" @click="handleConfirm">{{ confirmText || translate('confirm') }}</wui-button>
      </view>
    </wui-action-sheet>
  </view>
</template>
...
import type { CalendarType } from '../wui-calendar-view/types'
