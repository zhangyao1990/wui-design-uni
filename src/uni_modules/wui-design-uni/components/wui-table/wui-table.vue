<template>
  <view :class="`wui-table ${border ? 'is-border' : ''} ${customClass}`" :style="tableStyle">
    <scroll-view
      :enable-flex="true"
      :throttle="false"
      :scrollLeft="reactiveState.scrollLeft"
      :scroll-x="true"
      class="wui-table__header"
      @scroll="scroll"
      v-if="showHeader"
    >
      <view id="table-header" class="wui-table__content" :style="realWidthStyle" style="position: sticky; top: 0; z-index: 2">
        <view
          :class="`wui-table__cell ${border ? 'is-border' : ''} ${column.fixed ? 'is-fixed' : ''} ${stripe ? 'is-stripe' : ''} is-${column.align} ${
        ...
        >
          ...
          />
          <text v-else :class="`wui-table__value ${ellipsis ? 'is-ellipsis' : ''}`">{{ column.label }}</text>
        </view>
      </view>
    </scroll-view>
    <scroll-view
      class="wui-table__body"
      :style="bodyStyle"
      :enable-flex="true"
      :throttle="false"
      :scroll-x="true"
      ...
      :scrollLeft="reactiveState.scrollLeft"
    >
      <view id="table-body" class="wui-table__content" :style="realWidthStyle">
        <wui-table-col
          v-if="index !== false"
...
import wuiTableCol from '../wui-table-col/wui-table-col.vue'
import wuiSortButton from '../wui-sort-button/wui-sort-button.vue'
...
import type { SortDirection, TableColumn, TableColumnInstance, TableColumnProps } from '../wui-table-col/types'
