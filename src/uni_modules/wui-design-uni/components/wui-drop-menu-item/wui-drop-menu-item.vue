<template>
  <view v-if="showWrapper" :class="`wui-drop-item  ${customClass}`" :style="`z-index: ${zIndex}; ${positionStyle};${customStyle}`">
    <wui-popup
      v-model="showPop"
      :z-index="zIndex"
      :duration="duration"
...
watch(
  ...
  (newValue) => {
    if (isDef(newValue) && typeof newValue !== 'number' && typeof newValue !== 'string') {
      console.error('[wui-design] warning(wui-drop-menu-item): the type of value should be a number or a string.')
    }
  },
  {
    deep: true,
