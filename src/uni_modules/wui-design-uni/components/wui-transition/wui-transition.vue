...
const getClassNames = (name?: TransitionName | TransitionName[]) => {
  ...
  if (Array.isArray(name)) {
    for (let index = 0; index < name.length; index++) {
      enter = `wui-${name[index]}-enter wui-${name[index]}-enter-active ${enter}`
      enterTo = `wui-${name[index]}-enter-to wui-${name[index]}-enter-active ${enterTo}`
      leave = `wui-${name[index]}-leave wui-${name[index]}-leave-active ${leave}`
      leaveTo = `wui-${name[index]}-leave-to wui-${name[index]}-leave-active ${leaveTo}`
    }
  } else if (name) {
    enter = `wui-${name}-enter wui-${name}-enter-active ${enter}`
    enterTo = `wui-${name}-enter-to wui-${name}-enter-active ${enterTo}`
    leave = `wui-${name}-leave wui-${name}-leave-active ${leave}`
    leaveTo = `wui-${name}-leave-to wui-${name}-leave-active ${leaveTo}`
  }
  return {
    enter: enter,
    'enter-to': enterTo,
