<template>
  <view
    ...
    :class="`wui-fab ${customClass}`"
    :style="rootStyle"
    @click.stop=""
  >
    <view @click.stop="" :style="{ visibility: inited ? 'visible' : 'hidden' }" id="trigger">
      <slot name="trigger" v-if="$slots.trigger"></slot>
      <wui-button v-else @click="handleClick" custom-class="wui-fab__trigger" round :type="type" :disabled="disabled">
        <wui-icon custom-class="wui-fab__icon" :name="isActive ? activeIcon : inactiveIcon"></wui-icon>
      </wui-button>
    </view>
    <wui-transition
      v-if="expandable"
      :enter-class="`wui-fab__transition-enter--${fabDirection}`"
      enter-active-class="wui-fab__transition-enter-active"
      :leave-to-class="`wui-fab__transition-leave-to--${fabDirection}`"
      leave-active-class="wui-fab__transition-leave-active"
