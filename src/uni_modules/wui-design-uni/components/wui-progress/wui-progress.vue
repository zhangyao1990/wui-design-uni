<template>
  <view :class="`wui-progress ${customClass}`" :style="customStyle">
    <!--进度条-->
    <view class="wui-progress__outer">
      <view :class="`wui-progress__inner ${progressClass}`" :style="rootStyle"></view>
    </view>
    <!--文案、图标-->
    <view v-if="!hideText" class="wui-progress__label">{{ percentage }}%</view>
    <wui-icon
      v-else-if="status"
      :custom-class="`wui-progress__label wui-progress__icon ${progressClass}`"
