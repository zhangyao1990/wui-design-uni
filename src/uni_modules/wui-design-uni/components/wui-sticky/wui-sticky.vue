<template>
  <view :style="`${rootStyle};display: inline-block;`">
    <view :class="`wui-sticky ${customClass}`" :style="stickyStyle" :id="styckyId">
      <view class="wui-sticky__container" :style="containerStyle">
        <wui-resize @resize="handleResize" custom-style="display: inline-block;">
          <slot />
        </wui-resize>
      </view>
    </view>
...
const styckyId = ref<string>(`wui-sticky${uuid()}`)
...
function handleRelativeTo({ boundingClientRect }: any) {
  // sticky 高度大于或等于 wui-sticky-box，使用 wui-sticky-box 无任何意义
