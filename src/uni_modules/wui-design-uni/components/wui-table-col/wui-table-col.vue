<template>
  <view
    :class="`wui-table-col ${fixed ? 'wui-table-col--fixed' : ''} ${isLastFixed && isDef(table) && table.scrollLeft ? 'is-shadow' : ''}`"
    :style="columnStyle"
  >
    <view
      :class="`wui-table__cell ${stripe && isOdd(index) ? 'is-stripe' : ''} ${border ? 'is-border' : ''} is-${align}`"
    ...
    >
      ...
      <text :class="`wui-table__value ${ellipsis ? 'is-ellipsis' : ''}`" v-else>{{ row }}</text>
    </view>
  </view>
</template>
...
import { TABLE_KEY } from '../wui-table/types'
