<template>
  <view :class="`wui-swiper ${customClass}`" :style="customStyle">
    <swiper
      :adjust-height="adjustHeight"
      :adjust-vertical-height="adjustVerticalHeight"
      class="wui-swiper__track"
    ...
    >
      <swiper-item v-for="(item, index) in list" :key="index" class="wui-swiper__item" @click="handleClick(index, item)">
        <image
          v-if="isImageUrl(isObj(item) ? item[valueKey] : item)"
          :src="isObj(item) ? item[valueKey] : item"
          :class="`wui-swiper__image ${customImageClass} ${customItemClass} ${getCustomItemClass(navCurrent, index, list)}`"
          :style="{ height: addUnit(height) }"
          :mode="imageMode"
        />
        <video
          ...
          :class="`wui-swiper__video ${customItemClass} ${getCustomItemClass(navCurrent, index, list)}`"
          ...
          objectFit="cover"
        />
        <text v-if="isObj(item) && item[textKey]" :class="`wui-swiper__text ${customTextClass}`" :style="customTextStyle">{{ item[textKey] }}</text>
      </swiper-item>
    </swiper>
