<template>
  <view :class="rootClass" :style="customStyle">
    <view v-if="label || useLabelSlot" :class="labelClass" :style="labelStyle">
      <view v-if="prefixIcon || usePrefixSlot" class="wui-textarea__prefix">
        <wui-icon v-if="prefixIcon && !usePrefixSlot" custom-class="wui-textarea__icon" :name="prefixIcon" @click="onClickPrefixIcon" />
        <slot v-else name="prefix"></slot>
      </view>
      <view class="wui-textarea__label-inner">
    ...
    <!-- 文本域 -->
    <view :class="`wui-textarea__value ${showClear ? 'is-suffix' : ''} ${customTextareaContainerClass} ${showWordCount ? 'is-show-limit' : ''}`">
      <textarea
        :class="`wui-textarea__inner ${customTextareaClass}`"
        v-model="inputValue"
        :show-count="false"
...
const rootClass = computed(() => {
  return `wui-textarea   ${props.label || props.useLabelSlot ? 'is-cell' : ''} ${props.center ? 'is-center' : ''} ${
...
const labelClass = computed(() => {
  return `wui-textarea__label ${props.customLabelClass} ${isRequired.value ? 'is-required' : ''}`
})

const inputPlaceholderClass = computed(() => {
  return `wui-textarea__placeholder  ${props.placeholderClass}`
})

const countClass = computed(() => {
  return `${currentLength.value > 0 ? 'wui-textarea__count-current' : ''} ${currentLength.value > props.maxlength ? 'is-error' : ''}`
})
