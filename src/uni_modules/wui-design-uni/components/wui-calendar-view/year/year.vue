  ...
  <view class="wui-year year">
    ...
    <view class="wui-year__months">
      ...
      >
        ...
        <view class="wui-year__month-text">{{ getMonthLabel(item.date) }}</view>
        <view class="wui-year__month-bottom">{{ item.bottomInfo }}</view>
      </view>
    </view>
  </view>
</template>
...
function setMonths() {
  ...
  if (props.type.indexOf('range') > -1 && value && !isArray(value)) {
    console.error('[wui-design] value should be array when type is range')
    return
  }
