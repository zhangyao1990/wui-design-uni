<template>
  <view>
    <wui-toast selector="wui-month" />
    <view class="month">
      <view class="wui-month">
        <view class="wui-month__title">{{ monthTitle(date) }}</view>
        <view class="wui-month__days">
          <view
            v-for="(item, index) in days"
            :key="index"
            :class="`wui-month__day ${item.disabled ? 'is-disabled' : ''} ${item.type ? itemClass(item.type, value!, type) : ''}`"
            :style="firstDayStyle(index, item.date, firstDayOfWeek)"
            @click="handleDateClick(index)"
          >
            <view class="wui-month__day-container">
              <view class="wui-month__day-top">{{ item.topInfo }}</view>
              <view class="wui-month__day-text">
                {{ item.text }}
              </view>
              <view class="wui-month__day-bottom">{{ item.bottomInfo }}</view>
            </view>
          </view>
        </view>
      </view>
...
const toast = useToast('wui-month')
...
function getFormatterDate(date: number, day: string | number, type?: CalendarDayType) {
  ...
  if (props.formatter) {
    if (isFunction(props.formatter)) {
      ...
      console.error('[wui-design] error(wui-calendar-view): the formatter prop of wui-calendar-view should be a function')
    }
  }
  return dayObj
}
