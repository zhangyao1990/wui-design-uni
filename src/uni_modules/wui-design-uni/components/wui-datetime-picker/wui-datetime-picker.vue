<template>
  <view
    :class="`wui-picker ${disabled ? 'is-disabled' : ''} ${size ? 'is-' + size : ''}  ${cell.border.value ? 'is-border' : ''} ${
      alignRight ? 'is-align-right' : ''
    } ${error ? 'is-error' : ''} ${customClass}`"
    :style="customStyle"
  >
    <!--文案-->
    <view class="wui-picker__field" @click="showPopup">
      <slot v-if="useDefaultSlot"></slot>
      <view v-else :class="['wui-picker__cell', customCellClass]">
        <view
          v-if="label || useLabelSlot"
          :class="`wui-picker__label ${customLabelClass} ${isRequired ? 'is-required' : ''}`"
          :style="labelWidth ? 'min-width:' + labelWidth + ';max-width:' + labelWidth + ';' : ''"
        >
          <block v-if="label">{{ label }}</block>
          <slot v-else name="label"></slot>
        </view>
        <view class="wui-picker__body">
          <view class="wui-picker__value-wraper">
            <view :class="`wui-picker__value ${customValueClass}`">
              <template v-if="region">
                <view v-if="isArray(showValue)">
                  <text :class="showValue[0] ? '' : 'wui-picker__placeholder'">
                    {{ showValue[0] ? showValue[0] : placeholder || translate('placeholder') }}
                  </text>
                  {{ translate('to') }}
                  <text :class="showValue[1] ? '' : 'wui-picker__placeholder'">
                    {{ showValue[1] ? showValue[1] : placeholder || translate('placeholder') }}
                  </text>
                </view>
                <view v-else class="wui-picker__placeholder">
                  {{ placeholder || translate('placeholder') }}
                </view>
              </template>
              <view v-else :class="showValue ? '' : 'wui-picker__placeholder'">
                {{ showValue ? showValue : placeholder || translate('placeholder') }}
              </view>
            </view>
            <wui-icon v-if="!disabled && !readonly" custom-class="wui-picker__arrow" name="arrow-right" />
          </view>
          <view v-if="errorMessage" class="wui-picker__error-message">{{ errorMessage }}</view>
        </view>
      </view>
    </view>
    ...
    <wui-popup
      ...
      :z-index="zIndex"
      @close="onCancel"
      custom-class="wui-picker__popup"
    >
      <view class="wui-picker__wraper">
        <!--toolBar-->
        <view class="wui-picker__toolbar" @touchmove="noop">
          <!--取消按钮-->
          <view class="wui-picker__action wui-picker__action--cancel" @click="onCancel">
            {{ cancelButtonText || translate('cancel') }}
          </view>
          <!--标题-->
          <view v-if="title" class="wui-picker__title">{{ title }}</view>
          <!--确定按钮-->
          <view :class="`wui-picker__action ${loading || isLoading ? 'is-loading' : ''}`" @click="onConfirm">
            {{ confirmButtonText || translate('confirm') }}
          </view>
        </view>
        <!-- 区域选择tab展示 -->
        <view v-if="region" class="wui-picker__region-tabs">
          <view :class="`wui-picker__region ${showStart ? 'is-active' : ''} `" @click="tabChange">
            <view>{{ translate('start') }}</view>
            <view class="wui-picker__region-time">{{ showTabLabel[0] }}</view>
          </view>
          <view :class="`wui-picker__region ${showStart ? '' : 'is-active'}`" @click="tabChange">
            <view>{{ translate('end') }}</view>
            <view class="wui-picker__region-time">{{ showTabLabel[1] }}</view>
          </view>
        </view>
        <!--datetimePickerView-->
        <view :class="showStart ? 'wui-picker__show' : 'wui-picker__hidden'">
          <wui-datetime-picker-view
        ...
        <view :class="showStart ? 'wui-picker__hidden' : 'wui-picker__show'">
          <wui-datetime-picker-view
            :custom-class="customViewClass"
...
import wuiDatetimePickerView from '../wui-datetime-picker-view/wui-datetime-picker-view.vue'
...
} from '../wui-datetime-picker-view/types'
