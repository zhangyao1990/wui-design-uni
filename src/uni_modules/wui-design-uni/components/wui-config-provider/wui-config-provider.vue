 ...
 * @Description: 
 * @FilePath: \wui-design-uni\src\uni_modules\wui-design-uni\components\wui-config-provider\wui-config-provider.vue
 * 记得注释
-->
<template>
...
const themeClass = computed(() => {
  return `wui-theme-${props.theme} ${props.customClass}`
})
...
const mapThemeVarsToCSSVars = (themeVars: Record<string, string>) => {
  ...
  Object.keys(themeVars).forEach((key) => {
    cssVars[`--wui-${kebabCase(key)}`] = themeVars[key]
  })
