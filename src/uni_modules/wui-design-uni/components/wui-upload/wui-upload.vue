<template>
  <view :class="['wui-upload', customClass]" :style="customStyle">
    <!-- 预览列表 -->
    <view :class="['wui-upload__preview', customPreviewClass]" v-for="(file, index) in uploadFiles" :key="index">
      <!-- 成功时展示图片 -->
      <view class="wui-upload__status-content">
        <image v-if="isImage(file)" :src="file.url" :mode="imageMode" class="wui-upload__picture" @click="onPreviewImage(file)" />
        <template v-else-if="isVideo(file)">
          <view class="wui-upload__video" v-if="file.thumb" @click="onPreviewVideo(file)">
            <image :src="file.thumb" :mode="imageMode" class="wui-upload__picture" />
            <wui-icon name="play-circle-filled" custom-class="wui-upload__video-paly"></wui-icon>
          </view>
          <view v-else class="wui-upload__video" @click="onPreviewVideo(file)">
            <!-- #ifdef APP-PLUS || MP-DINGTALK -->
            <wui-icon custom-class="wui-upload__video-icon" name="video"></wui-icon>
            <!-- #endif -->
            <!-- #ifndef APP-PLUS -->
            <!-- #ifndef MP-DINGTALK -->
            <video
        ...
        <view v-else class="wui-upload__file" @click="onPreviewFile(file)">
          <wui-icon name="file" custom-class="wui-upload__file-icon"></wui-icon>
          <view class="wui-upload__file-name">{{ file.name || file.url }}</view>
        </view>
      </view>

      <view v-if="file[props.statusKey] !== 'success'" class="wui-upload__mask wui-upload__status-content">
        <!-- loading时展示loading图标和进度 -->
        <view v-if="file[props.statusKey] === 'loading'" class="wui-upload__status-content">
          <wui-loading :type="loadingType" :size="loadingSize" :color="loadingColor" />
          <text class="wui-upload__progress-txt">{{ file.percent }}%</text>
        </view>
        <!-- 失败时展示失败图标以及失败信息 -->
        <view v-if="file[props.statusKey] === 'fail'" class="wui-upload__status-content">
          <wui-icon name="close-outline" custom-class="wui-upload__icon"></wui-icon>
          <text class="wui-upload__progress-txt">{{ file.error || translate('error') }}</text>
        </view>
      </view>
      <!-- 上传状态为上传中时不展示移除按钮 -->
      <wui-icon
        ...
        name="error-fill"
        custom-class="wui-upload__close"
    ...
    <block v-if="showUpload">
      <view :class="['wui-upload__evoke-slot', customEvokeClass]" v-if="$slots.default" @click="handleChoose">
        <slot></slot>
      </view>
      <!-- 唤起项 -->
      <view v-else @click="handleChoose" :class="['wui-upload__evoke', disabled ? 'is-disabled' : '', customEvokeClass]">
        <!-- 唤起项图标 -->
        <wui-icon class="wui-upload__evoke-icon" name="fill-camera"></wui-icon>
        <!-- 有限制个数时确认是否展示限制个数 -->
        <view v-if="limit && showLimitNum" class="wui-upload__evoke-num">（{{ uploadFiles.length }}/{{ limit }}）</view>
      </view>
    </block>
  </view>
  <wui-video-preview ref="videoPreview"></wui-video-preview>
</template>
...
import wuiVideoPreview from '../wui-video-preview/wui-video-preview.vue'
...
import type { VideoPreviewInstance } from '../wui-video-preview/types'
...
watch(
  ...
  (val) => {
    if (val && val < uploadFiles.value.length) {
      console.error('[wui-design]Error: props limit must less than fileList.length')
    }
  },
  {
    deep: true,
