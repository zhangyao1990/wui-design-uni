<template>
  <view
    :class="`wui-radio ${cellValue ? 'is-cell-radio' : ''} ${cellValue && shapeValue == 'button' ? 'is-button-radio' : ''} ${
  ...
  >
    ...
    <view class="wui-radio__shape" :style="isChecked && !disabledValue ? 'color: ' + checkedColorValue : ''">
      <wui-icon v-if="shapeValue === 'check'" :style="isChecked && !disabledValue ? 'color: ' + checkedColorValue : ''" name="check"></wui-icon>
    </view>
  </view>
</template>
<script lang="ts">
...
import { RADIO_GROUP_KEY } from '../wui-radio-group/types'
