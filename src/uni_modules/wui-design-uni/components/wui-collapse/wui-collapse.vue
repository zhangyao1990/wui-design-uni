<template>
  <view :class="`wui-collapse ${viewmore ? 'is-viewmore' : ''} ${customClass}`" :style="customStyle">
    <!-- 普通或手风琴 -->
    <block v-if="!viewmore">
      <slot></slot>
    </block>
    ...
    <view v-else>
      <view
        :class="`wui-collapse__content ${!modelValue ? 'is-retract' : ''} `"
        :style="`-webkit-line-clamp: ${contentLineNum}; -webkit-box-orient: vertical`"
      >
        <slot></slot>
      </view>
      <view class="wui-collapse__more" @click="handleMore">
        <!-- 自定义展开按钮 -->
        ...
        <!-- 显示展开或折叠按钮 -->
        <block v-else>
          <span class="wui-collapse__more-txt">{{ !modelValue ? translate('expand') : translate('retract') }}</span>
          <view :class="`wui-collapse__arrow ${modelValue ? 'is-retract' : ''}`">
            <wui-icon name="arrow-down"></wui-icon>
          </view>
        </block>
      </view>
    </view>
